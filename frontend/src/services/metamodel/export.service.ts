...
class ExportService {
  ...
  async exportMetamodel(metamodelId: string): Promise<boolean> {
    ...
    switch (format) {
      ...
      case 'png':
        // PNG export will be handled by the VisualMetamodelEditor via a callback
        if (typeof (window as any).exportMetamodelAsPng === 'function') {
          await (window as any).exportMetamodelAsPng(metamodelId);
          return true;
        } else {
  ...
  private async promptForExportFormat(metamodelName: string): Promise<ExportFormat | null> {
    return new Promise<ExportFormat | null>((resolve) => {
      ...
      pngButton.textContent = 'PNG Image';
      pngButton.style.padding = '8px 16px';
      pngButton.style.backgroundColor = '#222';
      pngButton.style.color = 'white';
      pngButton.style.border = 'none';
      pngButton.style.borderRadius = '4px';
      pngButton.style.cursor = 'pointer';
      pngButton.style.fontSize = '16px';
      pngButton.style.flex = '1';
      pngButton.style.margin = '0 5px 10px 0';
      pngButton.style.minWidth = '100px';
      pngButton.onclick = () => {
        document.body.removeChild(modal);
        resolve('png');
      };
