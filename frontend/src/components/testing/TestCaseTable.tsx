...
const TestCaseRow: React.FC<TestCaseRowProps> = ({ testCase }) => {
  ...
  const handleViewDetails = () => {
    // Pass test case data via URL using base64 encoding (no localStorage)
    const testCaseData = {
      id: testCase.id,
      name: testCase.name,
    ...
    // Encode test data as base64 for URL
    const encodedData = btoa(encodeURIComponent(JSON.stringify(testCaseData)));

    // Open in new tab with encoded data in URL
    window.open(`/test-details?data=${encodedData}`, '_blank');
  };
