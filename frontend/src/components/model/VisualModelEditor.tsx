...
import { useReferenceDrawing } from './hooks/useReferenceDrawing';
import { useModelData } from './hooks/useModelData';
import { ModelZoomControls } from './components/toolbar/ModelZoomControls';
...
const VisualModelEditor: React.FC<VisualModelEditorProps> = ({ modelId }) => {
  // Use reference drawing hook first (needed by useModelData)
  const {
    isDrawingReference,
  ...
  const { 
    ...
    handleStageDragMove,
    handleStageDragEnd
  } = useModelZoom(model, containerRef, stageRef);
  
  // Canvas state
  const [mousePos, setMousePos] = useState({ x: 0, y: 0 });
  
  // Use selection hook
  const { selectedElement, setSelectedElement, selectedModelReference, setSelectedModelReference } = useModelSelection();
  
  // Use validation hook
  const { validationIssues, setValidationIssues, isValidationDialogOpen, setIsValidationDialogOpen } = useModelValidation();
  
  // Use highlight hook
  const { highlightedElements, setHighlightedElements, handleHighlightAllResults, clearHighlights } = useModelHighlight(searchIndex);
  
  // Use search hook
  const { searchResults, setSearchResults, handleSearch, handleSelectSearchResult } = useModelSearch(
    searchIndex,
    model,
    scale,
    stageSize,
  ...
  // Get available attributes from metamodel for filter builder
  ...
  // Wrap zoom hook's drag handler to add isDrawingReference check and clear highlights
  const wrappedHandleStageDragStart = (e: any) => {
    // Only enable panning if not drawing a reference
    if (!isDrawingReference) {
      handleStageDragStart(e);
      // Also clear highlights when clicking on empty space
  ...
  const handleStageClick = (e: any) => {
    ...
    if (isDrawingReference && referenceStartElement) {
      ...
      const targetElement = findElementAtPosition(x, y, model);
  ...
  // Helper functions now use imported utilities
  ...
  const renderElement = (element: ModelElement) => {
    ...
    return (
      <Group
        ...
        onClick={() => {
          // Clear highlights when clicking on an element
          clearHighlights();
      ...
      >
        ...
        <Rect
          width={width}
          height={height}
          fill={isHighlighted ? "#fffacd" : "#fff"}
          stroke={isSelected ? "blue" : isHighlighted ? "#ffa500" : "#ccc"}
    ...
    for (const element of model.elements) {
      ...
      for (const [refName, refValue] of Object.entries(element.references)) {
        ...
        for (const targetId of targetIds) {
          ...
          const storedBendPoints = (element.references as any)[`${refName}_bendPoints`];
          const parsedBendPoints = storedBendPoints ? parseBendPoints(storedBendPoints) : undefined;
          ...
          // Calculate the points for drawing the reference line using utility
          const points = calculateReferencePath(element, targetElement, parsedBendPoints, isSelfReference);
    ...
    <Box sx={{ display: 'flex', height: 'calc(100vh - 64px)' }}>
      ...
      >
        <Stage
          ...
          onMouseDown={wrappedHandleStageDragStart}
        ...
        >
          ...
          <Box sx={{ width: 200 }}>
            <SearchBar
              ...
              onHighlightAll={handleHighlightAllResults}
